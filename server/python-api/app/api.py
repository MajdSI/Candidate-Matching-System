...
class MatchReq(BaseModel):
    ...
    # Async explanation: if True, return results immediately and fetch explanations separately
    async_explain: bool = False

class ExplainReq(BaseModel):
    jd_text: str
    candidates: List[Dict[str, Any]]  # List of candidate dicts with cv_text
...
def match(req: MatchReq):
    # Start timing for the entire match endpoint
    ...
    if AUTO_EXPLAIN:
        # If async_explain is True, return results immediately without explanations
        if req.async_explain:
            # Return results immediately - frontend can fetch explanations separately
            return out
...
@app.post("/explain", response_model=List[Dict[str, Any]])
def explain_candidates(req: ExplainReq):
    """Separate endpoint to fetch explanations for candidates asynchronously.
    
    This allows the frontend to:
    1. Show results immediately from /match endpoint
    2. Fetch explanations separately in the background
    3. Update UI progressively as explanations arrive
    
    IMPORTANT: The candidates list MUST be the top 5 ranked candidates from /match endpoint.
    The order and ranking are preserved - explanations are generated for the exact same candidates
    in the exact same order as returned by /match.
    """
    explain_start_time = time.time()
    
    # Verify we have candidates and log their ranks for debugging
    if not req.candidates:
        return []
    
    # Log the ranks to verify we're explaining the best 5
    ranks = [c.get("rank") for c in req.candidates if c.get("rank")]
    cv_uids = [c.get("cv_uid") for c in req.candidates]
    print(f"[EXPLAIN] Generating explanations for {len(req.candidates)} candidates")
    print(f"[EXPLAIN] Ranks: {ranks}")
    print(f"[EXPLAIN] CV UIDs: {cv_uids}")
    
    # IMPORTANT: Sort by rank to ensure order is preserved (in case frontend sends wrong order)
    # The candidates should already be in rank order, but we enforce it here
    sorted_candidates = sorted(req.candidates, key=lambda x: x.get("rank", 999))
    
    # Prepare shim for explainer
    shim = SimpleNamespace(
        jd_text=req.jd_text,
        llm_model=req.llm_model or LLM_MODEL_DEFAULT,
        max_reasons=req.max_reasons,
        per_cv_char_budget=req.per_cv_char_budget,
    )
    
    # Generate explanations - order is preserved in explain_matches function
    enriched = explain_matches(shim, sorted_candidates)
    
    # Verify order is preserved in output
    output_ranks = [e.get("rank") for e in enriched if e.get("rank")]
    print(f"[EXPLAIN] Output ranks (should match input): {output_ranks}")
    
    explain_end_time = time.time()
    explain_duration = round(explain_end_time - explain_start_time, 2)
    
    # Add timing to first result
    if enriched and enriched[0].get("explanation"):
        if isinstance(enriched[0]["explanation"], dict):
            enriched[0]["explanation"]["_explain_time_seconds"] = explain_duration
